...
public class Controller {
	...
	private void connectToStats(EithonPlugin eithonPlugin) {
		...
		if (this._statsPlugin != null && this._statsPlugin.isEnabled()) {
			...
			eithonPlugin.getEithonLogger().warning("EithonCop doesn't work without the EithonStats plugin");			
		}
	}
	...
	private void repeatRemindToCop() {
		final Controller thisObject = this;
		AlarmTrigger.get().repeat("EithonCop rank up reminders", Config.V.remindAfterMinutes*60, 
				new IRepeatable() {
			@Override
	...
	void remindToCop() {
		for (EithonPlayer eithonPlayer : this._knownPlayers.values()) {
			...
			if (expectedRankStartAtOne > currentRankStartAtOne) {
				remindToCop(player, expectedRankStartAtOne);
			}
		}
	}

	private void remindToCop(Player player, int expectedRankStartAtOne) {
		String rankGroup = this._rankGroupLadder.getPermissionGroup(expectedRankStartAtOne);
		Config.M.rememberToCop.sendMessage(player, rankGroup);
	}

	public void cop(Player player) {
		if (!this._rankGroupLadder.canUpdatePermissionGroups()) {
			player.sendMessage("EihonCop doesn't work without the zPermissions plugin");			
		} else {
		...
		if (this._statsPlugin == null) {
			player.sendMessage("EithonCop doesn't work without the EithonStats plugin");
		} else {
